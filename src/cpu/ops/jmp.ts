...
export const jp_cond_a16 = (cond: (cpu: CPU) => boolean): OpExec => (cpu, pc) => {
  if (cond(cpu)) {
    const nn = cpu.memory.read(pc + 1) | (cpu.memory.read(pc + 2) << 8);
    cpu.registers[REGISTER.PC] = nn;
  } else {
...
export const jr_cond_r8 = (cond: (cpu: CPU) => boolean): OpExec => (cpu, pc) => {
  if (cond(cpu)) {
    let n = cpu.memory.read(pc + 1);
    if (n & 0x80) {
      n = -(((~n) + 1) & 0xff);
...
export const call_cond_a16 = (cond: (cpu: CPU) => boolean): OpExec => (cpu, pc) => {
  if (cond(cpu)) {
    // push
    const value = pc + 3;
    const sp = cpu.registers[REGISTER.SP];
...
export const rst_nn = (n: number): OpExec => (cpu, pc) => {
  // push
  const value = pc + 2;
  const sp = cpu.registers[REGISTER.SP];
  cpu.memory.write(sp - 1, (value >>> 8) & 0xff);
...
export const ret_cond = (cond: (cpu: CPU) => boolean): OpExec => (cpu, pc) => {
  if (cond(cpu)) {
    // pop
    const sp = cpu.registers[REGISTER.SP];
    const value = cpu.memory.read(sp) | (cpu.memory.read(sp + 1) << 8);
