...
export const push = (r: Register16Description): OpExec => (cpu) => {
...
export const pop = (r: Register16Description): OpExec => (cpu) => {
  const sp = cpu.registers[REGISTER.SP];
...
export const add16 = (r1: Register16Description, r2: Register16Description): OpExec => (cpu, pc) => {
...
export const inc16 = (r: Register16Description): OpExec => (cpu) => {
  const n1 = r.read(cpu);
  const result = n1 + 1;
...
export const dec16 = (r: Register16Description): OpExec => (cpu) => {
  const n1 = r.read(cpu);
  const result = n1 - 1;
  r.write(cpu, result & 0xffff);
  cpu.skip(1);
